...
更新日時: 2026-09-06 09:40

## 現在の到達点

槍足軽に加え、刀武士・弓足軽・騎馬武者の3兵種を追加した。追加兵種は `tools/blender/generate_units.py` で既存の人体・描画関数を再利用し、武器・装具・馬体と動作を生成する。兵種確認で表示でき、刀武士は近接戦闘にも接続済み。弓と騎馬の固有戦闘は未接続。[兵種ごとの状況](unit_types.md)を参照。

追加3兵種の再生成:

```powershell
$assetScript = '.\tools\generate-units.ps1'
& $assetScript
```

`-Unit samurai` / `-Unit archer` / `-Unit cavalry` で対象を絞れる。専用のバックグラウンドBlenderプロセスを使い、各生成が成功した後に配布用PNG・JSONを `assets/sprites/` へコピーする。各モデルの `.blend` は `build/assets/<兵種>/` に保存する。新兵種の動作は生成スクリプトによるポーズ変換で、保存した `.blend` へのアニメーションのベイクは未実装。

各兵種につき `{兵種}_idle.png`（512×64）、`{兵種}_walk.png` と `{兵種}_attack.png`（512×512）、各JSONを生成する。兵種名は `samurai` / `archer` / `cavalry`。歩兵の縮尺・Pivotは槍足軽と共通。騎馬は待機・移動の縮尺5、攻撃6、Pivotはいずれも `(0.5, 0.75)`。`src/unit_type.h` が実行時の描画定義を持つ。兵種切り替え時に同じAtlas領域へ読み直し、複数兵種の同時描画はまだ行わない。

`unit_assets` テストで追加408コマの透過・画像端・動作差、攻撃／移動の寸法・Pivotを検証する。兵種別DX12テストではF4で一巡して元に戻ることも確認する。
