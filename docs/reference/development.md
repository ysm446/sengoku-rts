...
通常の戦場でF8、Spaceの順に押すと槍足軽・刀武士・弓足軽・騎馬武者の500対500を開始する。直接起動する場合:
...
Homeで混成編成のまま初期化する。F7で刀だけの戦闘へ切り替える。`dx12_mixed_battle` は混成戦闘の操作・初期化・素材切り替え・交戦・敗走と勝敗を検証する。弓は射撃と飛翔・着弾に対応し、騎馬80人ずつの接近・突撃にも対応する。[騎馬の仕様](cavalry_combat.md)を参照。[射撃の仕様](ranged_combat.md)を参照。[混成の仕様](melee_profiles.md)を参照。

## 関ヶ原デモ

通常起動は戦闘シミュレーション。`--sekigahara` を指定すると関ヶ原の史実再生デモを開く。再生デモの開発優先順位はシミュレーションより低い。タイムラインのドラッグ、局面クリック、再生・停止・速度変更、軍勢選択が可能。初期コンテンツ領域は1920×1080（枠・タイトルバーを除く）。WASD、ホイール、Q/E、中ドラッグでカメラを操作する。←/→は10分の時間移動。出典、模式化の範囲、全操作は[関ヶ原デモ](sekigahara_demo.md)を参照。

従来の戦闘試作は `--battle` または右上の「戦闘試作へ」。以下の操作一覧は戦闘試作のもの。

## 戦闘試作の操作

関ヶ原画面の「兵種を見る」から兵種確認へ移動できる。F4で槍足軽・刀武士・弓足軽・騎馬武者を切り替える。F3で移動／攻撃、Spaceで動作再生。直接起動は `--unit samurai` / `--unit archer` / `--unit cavalry`。刀武士はF7または `--sword-battle` で近接戦闘を開き、Spaceで進軍する。弓の射撃と騎馬の突撃はF8の混成戦へ接続済み。[近接戦闘の仕様](melee_profiles.md)を参照。
...
素材確認のF6、または `--formation --unit cavalry` で兵種別の隊列確認を開く。右クリックで移動命令、Spaceで再生、Hでその場の整列、Homeで初期化。F4で兵種を切り替える。[隊列の仕様](formation_drill.md)を参照。

- `formation_drill`：4兵種の移動・旋回・再整列・簡易衝突と、停止・更新頻度を検証。
- `melee_profiles`：刀と槍の間合い、向き、攻撃線の閉塞、個体の攻撃距離、固定刻みを検証。
- `dx12_sword_battle`：刀武士の描画・斬撃・局所敗走・全体撤退を検証。
- `dx12_formation`：騎馬隊列の10秒間の移動・方向変更を描画。入力と旋回中・終了時の画像を確認。
