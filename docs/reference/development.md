...
引数なし、または `--sekigahara` で関ヶ原の史実再生デモを開く。タイムラインのドラッグ、局面クリック、再生・停止・速度変更、軍勢選択が可能。初期画面は1600×960。WASD、ホイール、Q/E、中ドラッグでカメラを操作する。←/→は10分の時間移動。出典、模式化の範囲、全操作は[関ヶ原デモ](sekigahara_demo.md)を参照。
