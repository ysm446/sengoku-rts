...
通常描画では兵士のInstanceデータを毎フレーム更新し、地形やTextureは再転送しない。初期状態では既定の目的地へ進軍し、右クリックで選択部隊の移動先を変更できる。選択は地形上のクリック点と部隊中心の±14の範囲で判定し、重なった場合は近い中心を選ぶ。地形外のクリックは移動命令を出さない。隊列が地形をはみ出さないよう目的地の中心を±60へ制限する。接触は半幅14の正方形同士で判定する。経路探索・障害物回避・多部隊の衝突解決・追撃は未実装。
