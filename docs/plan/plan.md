...
- 槍足軽の簡易Rig、剛体ウェイト、Idleと8フレームWalkの8方向生成を実装した。[素材生成手順](../reference/asset_pipeline.md)を参照。部隊単位の進軍・一時停止・到着時停止まで動作する。左クリックの部隊選択・右クリックの移動命令・Hの停止命令まで実装した。接触戦闘・損害・隊列と士気の低下・自動撤退まで実装した。次は攻撃素材と戦況の見やすさを調整する。[戦闘の仮ルール](../reference/combat_prototype.md)を参照。
...
- ユーザーの希望を受け、兵士ごとの追従位置・動作時計・生存状態と、倒れた場所に残る仮表現を実装した。[個体表示の設計](../reference/individual_soldiers.md)を参照。次は専用の攻撃・死亡素材と、前列や個体同士の重なりを調整する。個体単位の戦闘判定は未実装。
