...
通常起動は戦闘シミュレーションです。Spaceで槍足軽の進軍を開始し、F7で刀武士同士、F8で槍・刀・弓・騎馬の混成戦へ切り替えます。混成戦には両軍80人ずつの騎馬が参加し、助走による突撃を行います。F2で兵種素材、素材確認からF6で隊列確認を開けます。現在は兵種・隊列・衝突・士気による自律戦闘を優先して開発しています。
...
「兵種を見る」では槍足軽・刀武士・弓足軽・騎馬武者を確認できます。F4で兵種、F3で移動／攻撃を切り替え、Spaceで再生します。直接開く場合は `--unit samurai` / `--unit archer` / `--unit cavalry`。弓の射撃と騎馬の突撃はF8の混成戦で確認できます。
