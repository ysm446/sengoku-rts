...
void BattleSimulation::update(float seconds) {
    ...
    while (accumulator + 1e-8 >= fixedStep) {
        ...
        for (unsigned team = 0; team < 2; ++team) for (unsigned id = 0; id < 25; ++id) {
            ...
            if (g.cavalry.phase != CavalryPhase::None) { g.chargeDistance = g.chargeWindow = 0; continue; }
...
void BattleSimulation::updateSmallGroups(float seconds) {
    ...
    for (unsigned team = 0; team < 2; ++team) {
        ...
        for (unsigned sideIndex = 0; sideIndex < f.organization.frontReliefs.size(); ++sideIndex) {
                    ...
                    if (displaced && !busy && !a.routed && !a.resting && !b.routed && !b.resting &&
                        ...
                        const auto clear=[&](Point from,Point to,Point stationary) {
                            ...
                            for(unsigned t=0;t<2;++t)for(unsigned other=0;other<25;++other) {
                                ...
                                auto obstacle=positions[t][other];
                                if(t==team && (supporters&(1u<<other))) {obstacle.x+=alongX?0:shift;obstacle.z+=alongX?shift:0;}
                                if(segmentDistance(from,to,obstacle)<clearance)return false;
                            }
                            return true;
                        };
                        ...
                        if ((alongX?head.x-rear.x:head.z-rear.z)*sign>=clearance && battleDistance(head,rear)<=16)
                            for(unsigned attempt=0;attempt<4;++attempt) {
                                const float side=attempt%2==0?preferred:-preferred;
                                supporters=0;shift=side<0?-6.1f:6.1f;
                                if(attempt>=2) {
                                    if(sideIndex<2)continue;
                                    bool movable=true;
                                    for(unsigned other=0;other<25;++other) {
                                        const auto& h=f.organization.smallGroups[other];
                                        const unsigned column=alongX?h.slot/5:h.slot%5;
                                        if(h.strength<=0 || (side<0?column>=lane:column<=lane))continue;
                                        if(h.routed || h.resting || h.route!=SmallGroupRoute::None || h.cavalry.phase!=CavalryPhase::None) {movable=false;break;}
                                        supporters|=1u<<other;
                                    }
                                    if(!movable || !supporters)continue;
                                    // 外側から順に移す。各移動時点の占有と移動後の交代経路を検査する。
                                    unsigned moved=0;
                                    for(int column=side<0?0:4;column>=0 && column<5 && movable;column+=side<0?1:-1)
                                        for(unsigned other=0;other<25 && movable;++other)if((supporters&(1u<<other)) &&
                                            static_cast<int>(alongX?f.organization.smallGroups[other].slot/5:f.organization.smallGroups[other].slot%5)==column) {
                                            const auto from=positions[team][other];const Point to{from.x+(alongX?0:shift),from.z+(alongX?shift:0)};
                                            if(std::abs(to.x)>76 || std::abs(to.z)>76) {movable=false;break;}
                                            for(unsigned t=0;t<2 && movable;++t)for(unsigned id=0;id<25;++id) {
                                                if((t==team && id==other) || formations[t].organization.smallGroups[id].strength<=0)continue;
                                                auto obstacle=positions[t][id];
                                                if(t==team && (moved&(1u<<id))) {obstacle.x+=alongX?0:shift;obstacle.z+=alongX?shift:0;}
                                                if(segmentDistance(from,to,obstacle)<clearance) {movable=false;break;}
                                            }
                                            moved|=1u<<other;
                                        }
                                    if(!movable)continue;
                                }
                                const Point lateralStep{alongX?0:side,alongX?side:0};
                                const Point headSide{head.x+lateralStep.x,head.z+lateralStep.z},rearSide{rear.x+lateralStep.x,rear.z+lateralStep.z};
                                ...
                                relief.corridorGroups=supporters;relief.corridorShift=supporters?shift:0;
                                if(supporters) {
                                    relief.phase=4;
                                    for(unsigned other=0;other<25;++other)if(supporters&(1u<<other)) {
                                        auto& h=f.organization.smallGroups[other];
                                        h.offsetX+=h.approachX;h.offsetZ+=h.approachZ;h.approachX=h.approachZ=0;
                                        relief.corridorOrigin[other]=alongX?h.offsetZ:h.offsetX;
                                        h.route=SmallGroupRoute::ReliefCorridor;h.routeAlongX=alongX;
                                        h.approachSpeed=h.chargeDistance=h.chargeWindow=0;h.detourTarget=-1;
                                    }
                                }
                                for(auto* g:{&a,&b}) {
            ...
            if (relief.phase >= 4) {
                ...
                for (unsigned id = 0; id < 25; ++id) if (relief.corridorGroups & (1u << id)) {
                    ...
                    positioned &= (relief.alongX ? g.offsetZ : g.offsetX) == relief.corridorOrigin[id]+(relief.phase == 4 ? relief.corridorShift : 0);
                }
            ...
            if (relief.displaced) {
                ...
                if(relief.phase==3 && battleDistance(positions[team][relief.reserve],head)<.001f) {
                    ...
                    a.resting=true;
                    if(relief.corridorGroups)relief.phase=5;else relief={};
                }
                continue;
            }
        ...
        for (unsigned id = 0; id < 25; ++id) {
            ...
            if (result != BattleResult::Ongoing || f.defeated()) {
                g.cavalry={};g.cavalry.handledCharge=g.charges;
            }
            const auto p = positions[team][id];
            ...
            if (g.route == SmallGroupRoute::None) {
                if (g.cavalry.phase != CavalryPhase::None) {
                    g.state = SmallGroupState::Waiting;
                    if (!f.maneuverEnabled || g.cavalry.phase == CavalryPhase::Regrouping) { g.approachSpeed = 0; continue; }
                    const auto target = g.cavalry.destination;
                    const float distance = battleDistance(p,target);
                    if (distance < .001f) { g.approachSpeed = 0; continue; }
                    const float clearance = 4.5f + 3 * movementLimit * seconds;
                    bool blocked = std::abs(target.x)>76 || std::abs(target.z)>76;
                    for (unsigned t=0;t<2;++t) for (unsigned other=0;other<25;++other) {
                        if ((t==team && other==id) || formations[t].organization.smallGroups[other].strength<=0) continue;
                        if (segmentDistance(p,target,positions[t][other])<clearance) blocked = true;
                    }
                    const auto movement = movementProfile(UnitType::Cavalry);
                    const float alignment = ((target.x-p.x)*std::cos(g.heading)+(target.z-p.z)*std::sin(g.heading))/distance;
                    const float desired = !blocked && alignment>=.95f ? std::max(0.0f,movement.speed-(f.moving?f.speed:0)) : 0;
                    g.approachSpeed += std::clamp(desired-g.approachSpeed,-movement.acceleration*2*seconds,movement.acceleration*seconds);
                    const float travel = std::min(distance,desired>0?g.approachSpeed*seconds:0);
                    const Point next{p.x+(target.x-p.x)*travel/distance,p.z+(target.z-p.z)*travel/distance};
            ...
            if (relieving) {
                ...
                if (relief.displaced && g.route!=SmallGroupRoute::ReliefCorridor) {
                    if(relief.phase>=4)continue;
                    ...
                    const float desired = relief.corridorOrigin[id]+(relief.phase == 5 ? 0 : relief.corridorShift);
                    ...
                    for (unsigned other = 0; other < 25; ++other) if (relief.corridorGroups & (1u << other)) {
                        ...
                        const float otherDesired=relief.corridorOrigin[other]+(relief.phase==5?0:relief.corridorShift);
                        if ((relief.phase == 5 ? inside : outside) && (routeX ? h.offsetZ : h.offsetX) != otherDesired) precedingReady = false;
                    }
...
void BattleSimulation::step(float seconds) {
    ...
    updateCavalryTactics(seconds);
    for (unsigned team=0;team<2;++team) for (unsigned id=0;id<25;++id) {
        auto& g = formations[team].organization.smallGroups[id];
        if (g.routed || g.resting || g.strength<=0) continue;
        const auto p = points[team][id];
        const float targetHeading = g.cavalry.phase == CavalryPhase::Disengaging ? std::atan2(g.cavalry.destination.z-p.z,g.cavalry.destination.x-p.x) :
            g.detourTarget >= 0 && g.route == SmallGroupRoute::None ? std::atan2(g.detourZ-p.z,g.detourX-p.x) :
    ...
    for (unsigned team = 0; team < 2; ++team) for (unsigned id = 0; id < 25; ++id) {
        ...
        localContact = true;
        if (g.cavalry.phase != CavalryPhase::None || g.route == SmallGroupRoute::Returning || g.route == SmallGroupRoute::ReliefReserve ||
...
void BattleSimulation::updateCavalryTactics(float seconds) {
    const float clearance = 4.5f + 3 * groupMovementLimit(formations) * seconds;
    for (unsigned team=0;team<2;++team) for (unsigned id=0;id<25;++id) {
        auto& f=formations[team]; auto& g=f.organization.smallGroups[id]; auto& tactic=g.cavalry;
        if (f.groupUnit(id)!=UnitType::Cavalry) continue;
        if (result!=BattleResult::Ongoing || f.defeated() || !f.maneuverEnabled || g.routed || g.resting ||
            g.strength<=0 || g.route!=SmallGroupRoute::None) {
        ...
        for (float angle:{0.0f,.785398163f,-.785398163f}) {
            const BattlePoint destination{p.x+10*(dx*std::cos(angle)-dz*std::sin(angle)),
                p.z+10*(dx*std::sin(angle)+dz*std::cos(angle))};
            bool clear=std::abs(destination.x)<=76 && std::abs(destination.z)<=76;
            for (unsigned t=0;t<2 && clear;++t) for (unsigned other=0;other<25;++other) {
                const auto& obstacle=formations[t].organization.smallGroups[other];
                if ((t==team && other==id) || obstacle.strength<=0) continue;
                const auto point=formations[t].groupPosition(other);
                if (segmentDistance(p,destination,point)<clearance ||
                    (t!=team && !obstacle.routed && battleDistance(destination,point)<10)) { clear=false;break; }
            }
            if (!clear) continue;
...
void BattleSimulation::updateDecisions() {
    for (unsigned team = 0; team < 2; ++team) for (unsigned id = 0; id < 25; ++id) {
        ...
        else if (g.cavalry.phase != CavalryPhase::None) decision = TacticalDecision::Recover;
...
void BattleSimulation::updateFaceDeployments(float seconds) {
    ...
    for (unsigned team = 0; team < 2; ++team) for (unsigned id = 0; id < 25; ++id) {
        ...
        const bool returning = g.cavalry.phase != CavalryPhase::None || g.route == SmallGroupRoute::Returning || g.route == SmallGroupRoute::ReliefReserve || g.route == SmallGroupRoute::ReliefWithdraw || g.route == SmallGroupRoute::ReliefCorridor;
